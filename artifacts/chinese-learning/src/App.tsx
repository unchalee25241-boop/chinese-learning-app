...
const STREAK_KEY = “easy_chinese_streak”;

function getTodayISO() { return new Date().toISOString().split(“T”)[0]; }
function getYesterdayISO() { const d = new Date(); d.setDate(d.getDate() - 1); return d.toISOString().split(“T”)[0]; }

function loadStreak(): StreakData {
try { const raw = localStorage.getItem(STREAK_KEY); if (!raw) return defaultStreak(); return JSON.parse(raw); } catch { return defaultStreak(); }
}
function saveStreak(data: StreakData) { localStorage.setItem(STREAK_KEY, JSON.stringify(data)); }
function defaultStreak(): StreakData { return { currentStreak: 0, longestStreak: 0, lastStudyDate: null, totalDaysStudied: 0, studiedToday: false }; }

function recalculate(data: StreakData): StreakData {
const today = getTodayISO(); const yesterday = getYesterdayISO();
if (!data.lastStudyDate) return data;
if (data.lastStudyDate === today) return { …data, studiedToday: true };
if (data.lastStudyDate === yesterday) return { …data, studiedToday: false };
return { …data, currentStreak: 0, studiedToday: false };
}

function useStreak() {
const [streak, setStreak] = useState<StreakData>(() => recalculate(loadStreak()));

useEffect(() => {
const updated = recalculate(loadStreak());
setStreak(updated); saveStreak(updated);
}, []);
...
if (prev.studiedToday) return prev;
const today = getTodayISO(); const yesterday = getYesterdayISO();
const continuing = prev.lastStudyDate === yesterday || prev.lastStudyDate === today;
const newStreak = continuing ? prev.currentStreak + 1 : 1;
...
saveStreak(updated); return updated;
});
}, []);
...
<button onClick={() => setScreen(“home”)} style={{ background: “rgba(255,255,255,0.25)”, border: “none”, borderRadius: 999, padding: “7px 14px”, color: “#fff”, fontWeight: 700, cursor: “pointer”, fontSize: 13 }}>← กลับ</button>
{/* ★ Streak compact ในหน้า category */}
<div style={{ background: “rgba(255,255,255,0.2)”, borderRadius: 999, padding: “5px 12px”, display: “flex”, alignItems: “center”, gap: 5 }}>
<span style={{ fontSize: 16 }}>🔥</span>
<span style={{ color: “#fff”, fontWeight: 800, fontSize: 14 }}>{streak.currentStreak}</span>
</div>
</div>
