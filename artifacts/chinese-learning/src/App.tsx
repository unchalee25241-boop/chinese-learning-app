...
export default function App() {
  ...
  const [searchQuery, setSearchQuery] = useState("");  const [searchQuery, setSearchQuery] = useState("");  const cat = categories.find(c => c.id === activeCat);  const searchResults = searchQuery.trim().length > 0   ? categories.flatMap(c =>       c.words         .filter(w =>           w.th.toLowerCase().includes(searchQuery.toLowerCase()) ||           w.zh.includes(searchQuery) ||           (w.zhCN ?? "").includes(searchQuery)         )         .map(w => ({ word: w, cat: c }))     )   : [];  const searchResults = searchQuery.trim().length > 0   ? categories.flatMap(c =>       c.words         .filter(w =>           w.th.toLowerCase().includes(searchQuery.toLowerCase()) ||           w.zh.includes(searchQuery) ||           (w.zhCN ?? "").includes(searchQuery)         )         .map(w => ({ word: w, cat: c }))     )   : [];
  ...
  <ModeToggle mode={mode} onToggle={toggleMode} />
     ...
     {isPremium && <div style={{ fontSize: 13, color: "rgba(255,255,255,0.9)", fontWeight: 700 }}>👑 Premium Member</div>}
      ...
      <div style={{ padding: "0 16px 32px", marginTop: 8 }}>
        ...
        {/* Search Bar */} <div style={{ position: "relative", marginBottom: 18 }}>   <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", fontSize: 18, pointerEvents: "none" }}>🔍</span>   <input     type="text"     placeholder="ค้นหาคำศัพท์ภาษาไทยหรือจีน..."     value={searchQuery}     onChange={e => setSearchQuery(e.target.value)}     style={{       width: "100%", boxSizing: "border-box",       background: dark.card, border: `1.5px solid ${searchQuery ? "#E8433A" : dark.border}`,       borderRadius: 16, padding: "13px 16px 13px 44px",       color: dark.text, fontSize: 15, fontFamily: "inherit", outline: "none",       transition: "border-color 0.2s",     }}   />   {searchQuery.length > 0 && (     <button onClick={() => setSearchQuery("")}       style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.15)", border: "none", borderRadius: "50%", width: 26, height: 26, color: "#fff", cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>       ✕     </button>   )} </div>  {/* Search Results */} {searchQuery.trim().length > 0 ? (   <div>     <div style={{ color: dark.subtext, fontSize: 12, fontWeight: 700, letterSpacing: 1, marginBottom: 10, textTransform: "uppercase" }}>       ผลลัพธ์ {searchResults.length} คำ     </div>     {searchResults.length === 0 ? (       <div style={{ textAlign: "center", padding: "40px 20px", color: dark.subtext }}>         <div style={{ fontSize: 40, marginBottom: 10 }}>🔍</div>         <div style={{ fontSize: 15, fontWeight: 700, color: dark.text }}>ไม่พบคำศัพท์</div>         <div style={{ fontSize: 13, marginTop: 4 }}>ลองค้นหาด้วยคำอื่น</div>       </div>     ) : (       searchResults.map(({ word: w, cat: c }, i) => (         <div key={i} style={{ background: `linear-gradient(135deg, ${c.color}22, ${c.color}11)`, borderRadius: 16, padding: "14px 16px", border: `1.5px solid ${c.color}55`, marginBottom: 10, cursor: "pointer" }}           onClick={() => { setActiveCat(c.id); setCatTab("vocab"); setScreen("category"); setSearchQuery(""); }}>           <div style={{ display: "flex", alignItems: "center", gap: 10 }}>             <span style={{ fontSize: 20 }}>{c.emoji}</span>             <div style={{ flex: 1 }}>               <div style={{ fontSize: 26, fontWeight: 900, color: "#fff" }}>                 {mode === "cn" ? (w.zhCN ?? w.zhSimplified ?? w.zh) : w.zh}               </div>               <div style={{ fontSize: 11, color: c.color + "cc" }}>                 {mode === "cn" ? (w.pinyinCN ?? w.pinyin) : `${w.zhuyin} • ${w.pinyin}`}               </div>               <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.9)", marginTop: 2 }}>{w.th}</div>             </div>             <span style={{ background: c.color + "44", color: "#fff", borderRadius: 999, padding: "3px 10px", fontSize: 10, fontWeight: 700 }}>{c.label}</span>           </div>         </div>       ))     )}   </div> ) : (   <>     <div style={{ color: dark.subtext, fontSize: 12, fontWeight: 700, letterSpacing: 1, marginBottom: 10, textTransform: "uppercase" }}>หมวดคำศัพท์</div>
        {categories.map(c => (
