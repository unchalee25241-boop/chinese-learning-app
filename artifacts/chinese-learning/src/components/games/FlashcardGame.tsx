...
export function FlashcardGame({ words, color, onStudied, mode, onMastery }: Props) {
  const [pool, setPool] = useState(() => [...words].sort(() => Math.random() - 0.5).slice(0, FLASHCARD_SIZE));
  ...
  const handleKnown = () => {
    ...
    setFlipped(false);
    setTimeout(() => {
      if (idx + 1 >= pool.length) setShowResult(true);
      else setIdx(i => i + 1);
    }, 150);
  };
  ...
  const restart = () => {
    setPool([...words].sort(() => Math.random() - 0.5).slice(0, FLASHCARD_SIZE));
  ...
  if (showResult) {
    ...
    return (
      <div style={{ textAlign: "center", padding: "32px 20px" }}>
        ...
        <div style={{ color: "rgba(255,255,255,0.5)", marginBottom: 20 }}>ดูครบ {pool.length} คำแล้ว</div>
  ...
  return (
    <div style={{ padding: "8px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 13 }}>{idx + 1}/{pool.length}</span>
      ...
      <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 999, height: 5, marginBottom: 20 }}>
        <div style={{ background: color, width: `${(idx / pool.length) * 100}%`, height: "100%", borderRadius: 999, transition: "width 0.3s" }} />
      </div>
