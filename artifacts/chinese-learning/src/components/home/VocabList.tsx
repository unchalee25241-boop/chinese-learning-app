          ...
          <div key={i}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                ...
                <button
                  onClick={e => { e.stopPropagation(); toggleFavorite(w.zh); }}
                  style={{ background: fav ? "#FFD70033" : "rgba(255,255,255,0.08)", border: fav ? "2px solid #FFD700" : "2px solid rgba(255,255,255,0.2)", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, transition: "all 0.2s", transform: fav ? "scale(1.1)" : "scale(1)" }}>
                  {fav ? "⭐" : "☆"}
                </button>
              </div>
            </div>
                ...
                {w.examples.map((ex, j) => (
                  <div key={j} style={{ marginBottom: 8, background: color + "22", borderRadius: 10, padding: "8px 10px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#fff", flex: 1 }}>{mode === "cn" ? (ex.zhCN ?? ex.zh) : ex.zh}</div>
                      <SpeakButton text={mode === "cn" ? (ex.zhCN ?? ex.zh) : ex.zh} color={color} lang={mode === "cn" ? "zh-CN" : "zh-TW"} />
                    </div>
