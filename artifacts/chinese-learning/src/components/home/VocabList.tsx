          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              ...
              {w.examples && w.examples.length > 0 && (
                <div onClick={() => setExpanded(expanded === i ? null : i)} style={{ fontSize: 11, color: `${color}cc`, cursor: "pointer", userSelect: "none" }}>
                  {expanded === i ? "▲ ซ่อน" : "▼ ตัวอย่าง"}
                </div>
              )}
            </div>
          </div>
          {expanded === i && w.examples && (
            <div style={{ marginTop: 12, borderTop: `1px solid ${color}33`, paddingTop: 10, zIndex: 1, position: "relative" }}>
              {w.examples.map((ex, j) => (
                <div key={j} style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>{ex.zh}</div>
                  <div style={{ fontSize: 11, color: `${color}cc` }}>{mode === "cn" ? ex.pinyin : `${ex.zhuyin} • ${ex.pinyin}`}</div>
                  <div style={{ fontSize: 13, color: "rgba(255,255,255,0.7)" }}>{ex.th}</div>
                </div>
              ))}
            </div>
          )}
