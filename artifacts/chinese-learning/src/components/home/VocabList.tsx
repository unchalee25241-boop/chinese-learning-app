...
interface Props { words: Word[]; isPremium: boolean; color: string; onUpgrade: () => void; }

export function VocabList({ words, isPremium, color, onUpgrade }: Props) {
    ...
    <div>
      {visible.map((w, i) => (
        <div key={i} style={{ background: "#1E1E30", borderRadius: 18, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, border: `1.5px solid ${color}33`, marginBottom: 10 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 34, fontWeight: 900, color: "#fff" }}>{w.zh}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>{w.zhuyin} • {w.pinyin}</div>
